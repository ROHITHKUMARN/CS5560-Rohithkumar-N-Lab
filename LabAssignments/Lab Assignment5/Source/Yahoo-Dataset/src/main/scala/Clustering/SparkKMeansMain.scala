...
object SparkKMeansMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val inputPath = Seq("data/TrainDataset/*")
    Logger.getRootLogger.setLevel(Level.WARN)

    val topic_output = new PrintStream("data/KMeans_result.txt")
    ...
    val predictions = model.predict(training(sc,paths = Seq("data/TrainDataset/*")))
    ...
    resultsA.foreach(f => {
     topic_output.println( f._1._2.split("\\?").mkString("\n"))
    })
      topic_output.close()
      sc.stop()
  }
  ...
  private def training(sc: SparkContext, paths: Seq[String]): (RDD[Vector]) = {
